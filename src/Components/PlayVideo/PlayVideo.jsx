...
const PlayVideo = () => {
    ...
    const fetch_video_data = async () => {
        ...
        await fetch(video_data_url).then(res => res.json()).then(data => {
            if (data.items && data.items.length > 0) {
                setapidata(data.items[0])
            } else {
                console.error("API Error or No Video Data:", data);
            }
        })
    ...
    const fetchOtherData = async () => {
        if (apidata && apidata.snippet && apidata.snippet.channelId) {
            ...
            await fetch(channel_data_url).then(response => response.json()).then(data => {
        ...
        await fetch(comments_url).then(response => response.json()).then(data => {
